  ...
  const GILSYS_HOME_NOTICES = [
    {
      ...
      body: [
        ...
        "・CPU / ランク戦関連",
        "",
        "ランク戦で登場するCPUに称号が付くようになりました。",
        "CPUの強さは称号ランクに応じて変化し、高ランクCPUほど強いAIで戦うようになります。",
        "",
        "CPU戦の職業選択画面で、相手CPUの強さを10段階またはランダムから選べるようになりました。",
        "",
        "CPUの行動AIを大きく調整しました。",
        "職業ごとの特徴を活かした行動を取りやすくなるよう、弓兵・盗賊・狂人・陰陽師・錬金術師・僧侶などの行動優先度を見直しました。",
        "",
        "CPU同士の自動対戦ログをもとに、職業ごとの勝率や相性を確認しながらAI調整を行えるようにしました。",
        "",
        "・バトル仕様 / 演出調整",
        "",
        "スキル演出とHP減少タイミングを調整しました。",
        "ダメージポップアップ演出に合わせてHPが減るようになりました。",
        "",
        "先攻1ターン目のコイン配布が発生していた不具合を修正しました。",
        "",
        "修理キットに、ぼろぼろ装備を1つ修復する効果を追加しました。",
        "対象の装備がない場合は、これまで通り耐久回復のみ行われます。",
        "",
        "・画面表示 / UI調整",
        "",
        "スマホ表示時の画面調整を見直しました。",
        "スマホだけ個別に画面配置を変える処理を減らし、固定画面を拡大縮小する形に統一しました。",
        "",
        "一部UIの重なりや表示位置を調整しました。",
        "CPU詳細選択ボタン、耐久バー、バフ表示、竜切り替えアイコンなどを見やすくしました。",
        "",
        "・不具合修正",
        "",
        "一部エフェクトや画像の向き、表示位置を修正しました。"
      ].join("\n")
    },
    {
      title: "ver5.17アップデート内容",
      body: `ver5.17アップデート内容

・演出変更

弓兵の矢攻撃演出をリニューアルしました。
矢が降ってくるアニメーションを追加し、攻撃回数や追撃回数に応じて表示される矢の本数が変化するようになりました。
また、装備している矢の種類に応じて矢の見た目も変化します。

騎士スキルの演出を調整しました。
スキルエフェクトのサイズや見た目を調整し、攻撃演出がより自然に見えるよう改善しました。

戦士スキル2の効果音を変更しました。
ヒット時の効果音と回復時の効果音を新しいSEに差し替え、演出に合うよう調整しました。

・仕様変更

弓兵の追撃強化バフの挙動を修正しました。
追撃強化バフが複数付与された場合、それぞれが個別に効果を発揮するようになりました。
例として、追撃+1のバフが2つ付与されている場合は追撃+2として扱われます。

・その他

バトル中のダメージ表示・効果音・エフェクト再生タイミングを細かく調整しました。`
    },
    {
